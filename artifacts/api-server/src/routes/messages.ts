...
async function canChat(currentUserId: number, targetUserId: number): Promise<boolean> {
  ...
  const [aFollowsB] = await db.select().from(userFollowsTable)
    .where(and(eq(userFollowsTable.followerId, currentUserId), eq(userFollowsTable.followingId, targetUserId))).limit(1);
  const [bFollowsA] = await db.select().from(userFollowsTable)
    .where(and(eq(userFollowsTable.followerId, targetUserId), eq(userFollowsTable.followingId, currentUserId))).limit(1);
...
router.get("/messages/conversations", requireAuth, async (req, res): Promise<void> => {
  ...
  res.json(((rows as any).rows as any[]).map((r: any) => {
    const partner = partnerMap.get(String(r.partner_id));
    return {
      userId: Number(r.partner_id), username: partner?.username ?? "Pengguna",
      photoUrl: partner?.photoUrl ?? null, lastMessage: r.message,
...
router.get("/messages/:userId/can-chat", requireAuth, async (req, res): Promise<void> => {
  ...
  res.json({ allowed: await canChat(req.user!.id, targetId) });
});
...
router.get("/messages/:userId", requireAuth, async (req, res): Promise<void> => {
  ...
  const rows = await db.select().from(privateMessagesTable)
    ...
    .orderBy(desc(privateMessagesTable.createdAt)).limit(200);
  await db.update(privateMessagesTable).set({ read: true })
  ...
  res.json(rows.reverse().map((r) => ({
    ...
    message: r.deleted ? "" : r.message, deleted: r.deleted ?? false,
    createdAt: r.createdAt.toISOString(), isMine: r.senderId === me,
  })));
});
...
router.post("/messages/:userId", requireAuth, async (req, res): Promise<void> => {
  ...
  const [row] = await db.insert(privateMessagesTable).values({
    senderId: req.user!.id, recipientId: targetId, message: parsed.data.message,
  }).returning();
  res.status(201).json({
    id: row.id, senderId: row.senderId, recipientId: row.recipientId,
    message: row.message, deleted: false, createdAt: row.createdAt.toISOString(), isMine: true,
  });
});
...
router.delete("/messages/msg/:id", requireAuth, async (req, res): Promise<void> => {
  ...
  if (msg.senderId !== req.user!.id) { res.status(403).json({ error: "Tidak diizinkan" }); return; }
  await db.update(privateMessagesTable).set({ deleted: true }).where(eq(privateMessagesTable.id, msgId));
  res.json({ ok: true });
});
