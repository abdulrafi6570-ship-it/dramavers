          ...
          {loading ? (
            ...
            <div className="flex flex-col justify-end min-h-full space-y-1">
              {msgs.map((m, i) => {
                ...
                const profileHref = `/users/${m.userId}`;
                      ...
                      <div className="max-w-[72%] flex flex-col items-end">
                        ...
                        {m.replyTo && (
                          <div className="px-2 py-1 rounded-lg border-r-2 border-white/25 bg-white/[0.04] text-xs text-white/35 max-w-full text-right mb-1">
                            ...
                            {" · "}
                            <span className="truncate">{m.replyTo.message || "pesan dihapus"}</span>
                          </div>
                        )}
                        <div
                          className={`rounded-2xl rounded-tr-sm px-3.5 py-2 ${m.deleted ? "bg-white/[0.05] border border-white/10" : ""}`}
                        ...
                        {/* Tombol hapus + reply — selalu tampil */}
                        {!m.deleted && (
                    ...
                    <div className="w-7 h-7 shrink-0 self-end">
                      {isFirstInGroup && (
                        <Link href={profileHref}>
                          <div className="w-7 h-7 rounded-full bg-white/10 flex items-center justify-center text-[11px] font-bold text-white cursor-pointer hover:opacity-80 transition-opacity overflow-hidden">
                            {m.photoUrl
