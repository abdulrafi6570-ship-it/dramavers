...
export default function MessagesInbox() {
  ...
  return (
    <div className="h-dvh bg-background flex flex-col overflow-hidden">
      <Navbar />
      <div className="max-w-2xl w-full mx-auto flex-1 flex flex-col overflow-hidden min-h-0">
        ...
        <div className="flex-1 overflow-y-auto min-h-0 pb-20">
          <Link
            ...
            className="flex items-center gap-3 px-4 py-3 hover:bg-white/5 active:bg-white/8 transition-colors"
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-white/50 truncate mt-0.5">
                {lastGlobal ? `${lastGlobal.username}: ${lastGlobal.message}` : "Ruang obrolan semua pengguna"}
              </p>
            </div>
          </Link>
          ...
          {dmLoading ? (
            <p className="text-white/40 text-sm px-4 py-4">Memuat...</p>
          ) : conversations.length === 0 ? (
            <div className="px-4 py-10 text-center">
              <MessageCircle className="w-8 h-8 text-white/10 mx-auto mb-2" />
              <p className="text-white/30 text-sm">Belum ada DM. <span className="text-primary">Tap Chat</span> di profil temanmu.</p>
            </div>
          ) : (
            conversations.map((c) => (
              <Link
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-white/50 truncate mt-0.5">{c.isMine ? "Kamu: " : ""}{c.lastMessage}</p>
                </div>
                {c.unreadCount > 0 && (
                  <span className="w-5 h-5 rounded-full bg-primary text-[10px] font-bold flex items-center justify-center text-white shrink-0">{c.unreadCount}</span>
                )}
              </Link>
            ))
          )}
