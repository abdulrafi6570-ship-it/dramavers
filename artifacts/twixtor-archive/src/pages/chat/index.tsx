...
export default function GlobalChat() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      <Navbar />
      <main
        className="flex-1 container mx-auto px-0 md:px-4 max-w-2xl flex flex-col"
              ...
              return (
                <div key={msg.id} className={`flex gap-2.5 group ${isMe ? "flex-row-reverse" : ""}`}>
                  ...
                  <div className={`max-w-[72%] flex flex-col gap-0.5 ${isMe ? "items-end" : "items-start"}`}>
                    {!isMe && !msg.deleted && (
                      <Link href={msg.userId ? `/users/${msg.userId}` : "#"}>
                        <span className="text-[11px] text-white/35 px-1 hover:text-white/60 transition-colors cursor-pointer">
                          {msg.username}
                        </span>
                      </Link>
                    )}
                    ...
                    {msg.replyTo && (
                      <div className={`px-2 py-1 rounded-lg border-l-2 border-white/25 bg-white/[0.04] text-xs text-white/35 max-w-full ${isMe ? "text-right border-l-0 border-r-2" : ""}`}>
                        ...
                        <span className="truncate">
                          {msg.replyTo.deleted ? "chat telah di hapus" : msg.replyTo.message}
                        </span>
                      </div>
                    )}
                    ...
                    <div className={`px-3 py-2 rounded-2xl text-sm leading-snug ${
                      msg.deleted
                        ? "bg-white/[0.03] text-white/20 italic border border-white/[0.04]"
                        : isMe
        ...
        <div className="fixed bottom-[4.5rem] md:bottom-0 left-0 right-0 md:relative border-t border-white/[0.06] bg-background px-4 py-3 flex-shrink-0 max-w-2xl mx-auto w-full">
          {!user ? (
            <div className="text-center text-sm text-white/30 py-2">
              <Link href="/login" className="text-white/60 hover:text-white underline">Masuk</Link>{" "}
              untuk ikut chat
            </div>
          ) : (
