              ...
              return (
                <div key={msg.id} className={`flex gap-2.5 ${isMe ? "flex-row-reverse" : ""}`}>
                  ...
                  <Link href={msg.userId ? `/users/${msg.userId}` : "#"}>
                    <div className="w-7 h-7 rounded-full bg-white/10 border border-white/10 flex items-center justify-center text-[11px] font-bold text-white/60 flex-shrink-0 cursor-pointer hover:bg-white/15 transition-colors">
                  ...
                  <div className={`max-w-[72%] ${isMe ? "items-end" : "items-start"} flex flex-col gap-0.5`}>
                    ...
                    <span className="text-[10px] text-white/20 px-1">{fmt(msg.createdAt)}</span>
                  </div>
                </div>
              );
            })
