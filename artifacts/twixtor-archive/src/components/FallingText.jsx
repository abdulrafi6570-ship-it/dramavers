...
const FallingText = ({
  ...
  useEffect(() => {
    ...
    const newHTML = words
      .map(word => {
        ...
        return `<span class="ft-word ${isHighlighted ? highlightClass : ''}">${word}</span>`;
      })
      .join(' ');
  ...
  useEffect(() => {
    ...
    const left    = Bodies.rectangle(-25, height / 2, 50, height, boundaryOptions);
    const right   = Bodies.rectangle(width + 25, height / 2, 50, height, boundaryOptions);
    const ceiling = Bodies.rectangle(width / 2, -25, width, 50, boundaryOptions);

    const wordSpans = textRef.current.querySelectorAll('.ft-word');
    ...
    World.add(engine.world, [floor, left, right, ceiling, mouseConstraint, ...wordBodies.map(wb => wb.body)]);
